...
%             end
                ...
                fai = -difbesseli0j(sqrt(j)*(m*q)) / difbesselk0j(sqrt(j)*m*q);
...
%         Zee(:,i) = Zee(:,i) - Zee(:,Noi);
%     end
    Zee(:,Noi+1:phaseinfo(k+1)-1) = Zee(:,Noi+1:phaseinfo(k+1)-1) - repmat(Zee(:,Noi), 1, phaseinfo(k+1)-1-Noi);
end
...
%         Zee(i,:) = Zee(i,:) - Zee(Noi,:);
%     end
    Zee((Noi+1): (phaseinfo(k+1) -1),:) = Zee((Noi+1): (phaseinfo(k+1) -1),:) - repmat( Zee(Noi,:),(phaseinfo(k+1) -1)-Noi ,1);
end
